...
```
s = √(2 · size · tan(180°/n) / n)
```

This comes from the area formula for a regular polygon: area = (1/2) · n · s · a with apothem *a*, and the relation between *s* and *a* below.
...
The game uses this for **click detection**: a click at (*p_x*, *p_y*) is inside the NGon if |*p_x* − *x*| ≤ *a* and |*p_y* − *y*| ≤ *a* (bounding-box style test).
...
- **Start:** (*x* − *s*/2, *y* − *a*)
- **For** *i* = 0, 1, …, *n*−1: add (*s* · cos(360° · *i* / *n*), *s* · sin(360° · *i* / *n*)) to get the next vertex.

So the polygon is drawn with one vertex at the top and edges of length *s* turning by 360°/*n* at each vertex.

### Color

Color is chosen from *n*: for *n* ≤ 16, RGB is (⌊210 · *n*/16⌋, 45, 210 − ⌊210 · *n*/16⌋) (red–blue gradient); for *n* > 16 the tile is drawn in blue.
